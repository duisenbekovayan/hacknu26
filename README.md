...
Файлы **`.env`** и **`.env.local.ps1`** содержат секретные ключи (API-ключи и т.д.) и **не добавлены в GitHub** (включены в `.gitignore`). Они размещены в **Google Drive** по ссылке, предоставленной вместе с проектом.
